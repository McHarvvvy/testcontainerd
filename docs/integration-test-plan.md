...
- **主动环境清理**：每个涉及 `Run()` 的用例（TC-08 ~ TC-17）在所有断言执行完毕后，必须主动清理环境残留产物——即使框架已自行回收，也通过 `t.Cleanup` 或 `defer` 兜底：强制终止 daemon 进程（如仍存活）、通过 Docker API 销毁已注册的容器（如仍存在）、删除 runtime.json（如仍存在）。这是测试隔离的保证，不是被测行为的断言。
...
- **仅 New 阶段的校验用例**（TC-01 ~ TC-07）不启动容器和 daemon，无需清理步骤。
...
### TC-06 注册项 Name 为空或 Start 为 nil
...
### TC-07 RegisterContainersFunc 自身返回 error
...
# defer/t.Cleanup 注册兜底清理：强制停止 daemon（如存活）、销毁容器 "redis"（如存在）、删除 runtime.json（如存在）
...
# defer/t.Cleanup 注册兜底清理：强制停止 daemon（如存活）、销毁容器 "redis-good"（如存在）、删除 runtime.json（如存在）
When  调用 Run(fakeM)
Then  Run 返回 1
And   fakeM.Run() 不会被调用
...
# defer/t.Cleanup 注册兜底清理：强制停止 daemon（如存活）、销毁容器 "redis-a" 和 "redis-b"（如存在）、删除 runtime.json（如存在）
When  调用 Run(fakeM)
Then  Run 返回 1
And   fakeM.Run() 不会被调用
